...
const (
        ...
        PENALTY_FOR_SKIPPING_IMPORTANT_LINE int = 10
)
...
func analyzeEditAndMapLine(source_lines, target_lines []string, source_lineno int) (int, error) {
        ...
        for i := range source_chars {
                ...
                // The penalty for not finding a match for a particular source character.
                // We will increase the penalty for characters that belong to the line we track.
                penaltyForSkippingThisChar := PENALTY_FOR_SKIPPING
                if i1 <= i && i < i2 {
                        penaltyForSkippingThisChar = PENALTY_FOR_SKIPPING_IMPORTANT_LINE
                }
